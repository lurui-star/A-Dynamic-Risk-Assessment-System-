...
@app.route('/')
def index():
    user = request.args.get('user', 'Guest')  # Default to 'Guest' if no user is provided
...
def predict():
    ...
    df = df.drop(['corporation', 'exited'], axis=1, errors='ignore')  # Ignore errors if columns not found
...
def score():
    ...
    Returns:
        str: model f1 score
    """
    output = subprocess.run(['python', 'src/scoring.py'], capture_output=True, text=True)
